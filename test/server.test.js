...
test("dashboard, sub-navbar docs, Scalar API reference, dan health tersedia", async (t) => {
  ...
  assert.match(mainStyles.body, /\.jobs-search-bar/);
  assert.match(mainStyles.body, /\.search-docs-input/);
  assert.match(mainStyles.body, /\.batch-action-bar/);
  assert.match(mainStyles.body, /\.job-card-checkbox/);
  assert.match(mainStyles.body, /\.job-card\.is-selected/);
  assert.match(page.body, /id="search-docs-input"/);
  assert.match(page.body, /id="search-clear-button"/);
  assert.match(page.body, /id="batch-action-bar"/);
  assert.match(page.body, /id="batch-select-all-checkbox"/);
  assert.match(appScript.body, /searchQuery/);
  assert.match(appScript.body, /selectedJobIds/);
  assert.match(appScript.body, /updateBatchActionBar/);
  assert.match(appScript.body, /batchMoveSelected/);
  assert.match(appScript.body, /batchDeleteSelected/);
